...
cwd = os.getcwd()
data_path = join(cwd, 'miniimagenet', 'images')
savedir = cwd
...
for dataset in dataset_list:
    with open('miniimagenet/' + datasetmap[dataset] + ".csv", "r") as lines:
        for i, line in enumerate(lines):
            ...
            #
            # fid = int(fid[-5:]) - 1
            fname_abs = join(data_path, fname)
            filelists[dataset][label].append(fname_abs)
...
for dataset in dataset_list:
    json_fname = join(savedir, dataset + ".json")
    fo = open(json_fname, "w")
